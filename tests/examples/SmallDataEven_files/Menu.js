	...
	**/
	...
	function Menu(Carrito, Cobertura) {
		...
		menuButtons.click(function() {
            ...
            modalEl.removeClass('show-image');
		...
		dataButtons.click(function (event, el) {
			event.preventDefault();
			$(this).addClass('loading');
			modalEl.addClass('show-image');
			addToCart($(this));
		...
		function addToCart(self) {
			...
			showInfo =  parseInt(self.attr('data-add'));
			dataStatus =  parseInt(self.attr('data-status'));
			restringir = parseInt(self.attr('data-restringir'));
		...
		function getProductConfigured() {
			...
			product = {
				...
				quantity: 1,
				restringir: info.data('restringir'),
			}
		...
		function menuConfig(menu) {
			...
			var restringir = 'data-restringir="0"';
			if(typeof menu.restringir !="undefined" && menu.restringir!='0'){
				    restringir= 'data-restringir="'+menu.restringir+'"';
				}
			...
			if(image != '' ) {
				...
				$('.modal').removeClass('no-image');
			}

			if(menu.grupoextras.length === 0 ) {
				$('.modal').addClass('no-extras');
			}

			var html ='';

			// html += '<h2 class="bold title" id="' + menu.id + '"  '+es_2x1+' '+restringir+' data-establecimiento="' + menu.establecimiento_id + '"  data-discount="' + menu.descuento_activo + '" data-name="' + menu.establecimiento + '" data-dom="' + menu.domicilio + '" data-cat="' + menu.categoria + '">' + menu.nombre + '</h2>\
			// 			<p class="desc">' + menu.descripcion + '</p>';
				if(dataStatus === 1) {
					...
					html += '<div class="menu-box">\
						...
						<h2 class="bold title" id="' + menu.id + '"  '+es_2x1+' '+restringir+' data-establecimiento="' + menu.establecimiento_id + '"  data-discount="' + menu.descuento_activo + '" data-name="' + menu.establecimiento + '" data-dom="' + menu.domicilio + '" data-cat="' + menu.categoria + '">' + menu.nombre + '</h2>\
						<p class="desc">' + menu.descripcion + '</p>\
						<ul>';
					...
					for(i = 0; i < menu.grupoextras.length; i++) {
						...
						if(total_extras > 4  && seleccion_multiple === true) {
							html += '<li class="clearfix large-items"><div class="item-info"><h4 class="red bold">' + extra.nombre + '</h4><p>' + seleccion_msg + '</p><div class="error-msg"></div></div><div class="item-group" data-extras="'+ total_extras +'" '+aplica_2x1+' data-id="' + extra.id + '" data-name="' + extra.nombre + '" data-min="' + min + '" data-max="' + max + '" data-multiple="' + ((seleccion_multiple) ? 1 : 0) + '">';
								...
								for(e = 0; e < extra.extras.length; e++) {
									...
									if(seleccion_multiple === false) {
										...
										if(min == total_extras || ( min>0 && min == max && min != 1)) {
											html += '<label><div class="add-more"><div class="minus button green">-</div><div class="in"><input readonly="readonly" type="text" data-price="' + item.precio + '" data-discount="' + item.discount + '" data-price-type="quantity" data-name="' + item.nombre + '" name="' + item.grupos_extras_id + '" id="' + item.id + '" type="text" value="0" /></div><div class="add button blue">+</div></div><div>' + item.nombre + '<span class="min-text"> + <span class="dinero">' + item.precio + '</span></div></label>';	
										} else {
							...
							html += '<li class="clearfix"><div class="item-info"><h4 class="red bold">' + extra.nombre + '</h4><p>' + seleccion_msg + '</p><div class="error-msg"></div></div><div class="item-group" data-extras="'+ total_extras +'" '+aplica_2x1+' data-id="' + extra.id + '" data-name="' + extra.nombre + '" data-min="' + min + '" data-max="' + max + '" data-multiple="' + ((seleccion_multiple) ? 1 : 0) + '">';
					...
					html += '</ul></div> <div class="total-box bottom"><span class="price light-gray ">Total: <span class="dinero" data-total="" data-init-price="' + precio + '"></span></span>';
					if(CoberturaModal.conCobertura() === 0 && es_franquicia) {
						html += '<div class="button green round5 add_cart">Verificar Cobertura</div></div>';

					}else {
						html += '<div class="button green round5 add_cart">Agregar</div></div>';
					}
				}else {
					return html;
				}
